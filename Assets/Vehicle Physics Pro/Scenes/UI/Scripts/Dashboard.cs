...
using VehicleObstacleDetect; // Harry: add for DetectObstacle namespace
...
public class Dashboard : MonoBehaviour
{
	...
	public Text laneChangeAlertLabel; // Harry: Add new dashboard label
	...
	void Update ()
	{
		...
		if (vehicle.time > m_lastVehicleTime)
		{
			...
			// Harry: ++ 0319 Add for lane changing alert
			laneChangeAlertLabel.text = "CHANGING LANE\n";
			if (detector.GetDetectStatus() == true)
			{
				// Use FL wheel's relative position for showing direction sign
				directionPos = GameObject.Find("WheelFL").transform.position;
				Quaternion directionRot = GameObject.Find("WheelFL").transform.rotation;

				// Determine normal from raycasting, we may know the possible direction to turn
				if (detector.GetSuggestedDirection() == detector.leftDirection)
				{
					Debug.Log("<< Turn Left");
					// Show overhead text for visual alert
					laneChangeAlertLabel.text = "CHANGING LANE\n<< Turn Left";

					// Only show one arrow during alert period
					if (directionSignLeft == null)
					{
						// Dismiss alert for the other direction 
						if (directionSignRight)
							Destroy(directionSignRight, 0.1f);
						
						// Rotate and locate arrow to the proper direction
						directionPos = new Vector3(directionPos.x, directionPos.y+1.5f, directionPos.z+10.0f); // TODO: This value is just for quick prototype, may have issue when in opposite direction. 
						// Create a new arrow 
						directionSignLeft = Instantiate(arrow, directionPos, heroCar.transform.rotation, heroCar.transform);
				...
				else if (detector.GetSuggestedDirection() == detector.rightDirection) 
				{
					...
					if (directionSignRight == null)
					{
						if (directionSignLeft)
							Destroy(directionSignLeft, 0.1f);
			...
			else
			{
				...
				if (directionSignLeft)
				{
					Destroy(directionSignLeft, 1.0f);
				}

				if (directionSignRight)
				{
					Destroy(directionSignRight, 1.0f);
				}
			} 
